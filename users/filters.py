...
class ProductFilter(django_filters.FilterSet):
    # Filter by sale percent (discount percentage)
    sale_percent = django_filters.NumberFilter(method='filter_sale_percent')

    # Already present: category and subcategory filters

    def filter_sale_percent(self, queryset, name, value):
        ...
        discount_expr = ExpressionWrapper(
            (Cast(F('original_price'), FloatField()) - Cast(F('sale_price'), FloatField()))
            / Cast(F('original_price'), FloatField()) * 100,
            output_field=FloatField()
        )
        return queryset.annotate(
            sale_percent_rounded=Round(discount_expr, 2)
        ).filter(
            is_on_sale=True,
            sale_price__isnull=False,
            original_price__gt=0,
            sale_percent_rounded__gte=value
        )
    ...
    def _split_parts(self, value):
        ...
        Normalizes common "empty" indicators sent by frontends (empty string, 'null', 'undefined')
        so that filters treat them as absent and do not apply restrictive queries.
        """
        ...
        if isinstance(value, (list, tuple)):
            ...
            # filter out common empty tokens
            parts = [p for p in parts if p.lower() not in ('', 'null', 'undefined')]
            return parts
    ...
    def filter_q(self, queryset, name, value):
        # Treat empty-like tokens from the frontend as no-op
        ...
        filtered = queryset.filter(
            Q(name_en_normalized__icontains=v_norm) |
            Q(name_ar_normalized__icontains=v_norm)
        )
